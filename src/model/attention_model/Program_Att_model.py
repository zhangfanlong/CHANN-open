...
class Program_Att_Net(nn.Module):
    def __init__(self, device, embedding_dim, hidden_dim, vocab_size, max_member_length,
        ...
        from src.model.attention_model.MemberAttNet_BiGRU import MemberAttNet
        self.member_att = MemberAttNet(self.hidden_dim, self.hidden_dim)

        from src.model.attention_model.EvoAttNet_LSTM import EvoAttNet
    ...
    def forward(self, input, member_len, life_len):
        ...
        evo_output, self.evo_hidden_state = self.evo_att(evo_input.permute(1, 0, 2), self.evo_hidden_state,
                                                             self.evo_c_state)
